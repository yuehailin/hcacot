...
class MYPromptLearner(nn.Module):
    ...
    def forward(self, feats, desc):
        ...
        p_ori = torch.cat([prefix, p_input, suffix], dim=1) #三者分别对应 prefix， global prompts 以及 low-level promtps [num_class*num_desc, max_token_length, dim]
        ...
        return p_ori, p_ins, p_uni, causal_attention_mask, attention_mask    #分别对应 [the first layer global prompts & low-level prompts] [low-level prompts] [other layers global prompts] [relatation attention]
...
class VisionPromptLearner(nn.Module):
    ...
    def forward(self, x):   
        x = x.type(self.dtype)  # [K, 3, 224, 224] K is the number of images
        x = self.embeddings(x)  # [K, 50, 768] 50 is the number of image patches (tokens)
        
        p_input = self.p_input.unsqueeze(0).expand(len(x), -1, -1) # [K, n_vpro, 768]
        x = torch.cat([x, p_input], dim=1)  # [K, 50+n_vpro, 768]
...
class VisionEncoder(nn.Module):
    ...
    def forward(self, x, p_visual):
        hidden_states = self.pre_layrnorm(x).type(self.dtype)   #  [K, 52, 768] K is the number of images, 52 is the number of patches (tokens)
        for layer_idx, encoder_layer in enumerate(self.layers):
            if layer_idx > 0:
                hidden_states[-self.n_vpro:] = p_visual[layer_idx-1].unsqueeze(1).expand(-1, hidden_states.shape[1], -1) #TODO 可能存在错误 改为 hidden_states[:,-self.n_vpro:] = p_visual[layer_idx-1].unsqueeze(0)
...
class TextEncoder(nn.Module):
    ...
    def forward(self, x, p_ins, p_uni, tokenized_prompts, attn, causal_attention_mask):  # x是第一层的global prompts和low-level prompts
        ...
        for layer_idx, layer in enumerate(self.num_layers):
            if layer_idx > 0:               
                ...
                # x = layer(x, attn[:, layer_idx].unsqueeze(1), causal_attention_mask)[0]
...
class TextEncoderZS(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        return out_put, txt_feats   #out_put应该是[N,512] txt_feats应该是[12, N,512]
