...
def collate_tranformer(batch):
	...
	img_f = torch.cat([item[4] for item in batch], dim = 0)
	label = torch.LongTensor([item[5] for item in batch])
	status = torch.LongTensor([item[6] for item in batch])
	time = torch.FloatTensor([item[7] for item in batch])
	disc = torch.LongTensor([item[8] for item in batch])
	soft_0 = torch.FloatTensor([item[9] for item in batch])
	soft_1 = torch.FloatTensor([item[10] for item in batch])
	soft_2 = torch.FloatTensor([item[11] for item in batch])
	soft_3 = torch.FloatTensor([item[12] for item in batch])
	
	


	return [img_s, coord_s, img_l, coord_l, img_f, label, status, time, disc, soft_0, soft_1, soft_2, soft_3]


def get_simple_loader(dataset, batch_size=1, num_workers=2, mode='clam'):
	if(mode == 'transformer'):
		collate = collate_tranformer

	kwargs = {'num_workers': 2, 'pin_memory': False, 'num_workers': num_workers} if device.type == "cuda" else {}
...
def get_optim(model, args):
	...
	elif args.opt.lower() == "ranger":
		optimizer = Ranger(filter(lambda p: p.requires_grad, model.parameters()),lr=args.lr,weight_decay=args.reg)
	else:
